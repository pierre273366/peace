...
router.get("/recuptricounts/:userToken", (req, res) => {
  ...
  User.findOne({ token: userToken }).then((user) => {
    ...
    Tricount.find({ participants: user._id }).then((tricounts) => {
...
router.get("/balances/:id", async (req, res) => {
  ...
  try {
    const tricount = await Tricount.findById(tricountId).populate({
      path: "expense",
      populate: [
...
router.delete("/delete/:tricountId", (req, res) => {
  ...
  User.findOne({ token: req.body.token }).then((user) => {
    ...
    Tricount.findById(req.params.tricountId).then((tricount) => {
      if (!tricount) {
      ...
      if (updatedParticipants.length === 0) {
        Tricount.deleteOne({ _id: req.params.tricountId }).then(() => {
          res.json({
            result: true,
            message: "Tricount supprimé car plus aucun participant",
          });
        });
      } else {
